...
showVotes <- function(df, x, co, region, N, minN) {
   ...
   if (minN < 1) {
   t <- sprintf("Wybory 2014. Głosy na %s\n%s ogółem %d komisji", co, region, kN ) } 
   else { t <- sprintf("Wybory 2014. Głosy na %s\n%s ogółem %d komisji (N>%d)", co, region, kN, minN ) } 

   p <- ggplot(df, aes(x=df[[x]])) + geom_density()
   p
...
showVotes(komisje, "SLDLRp", "SLD", region, kN, minTurnout);
